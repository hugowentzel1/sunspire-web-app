# Page snapshot

```yaml
- strong: Exclusive preview built for TestCompany
- text: "Exclusive preview — expires in 6:23:59:53 Runs left: 2"
- button "Launch on TestCompany"
- button "Copy link"
- button "Dismiss": ✕
- banner:
  - text: T
  - heading "TestCompany" [level=1]
  - paragraph: Solar Intelligence
  - navigation:
    - link "Enterprise":
      - /url: "#"
    - link "Partners":
      - /url: "#"
    - link "Support":
      - /url: "#"
    - button "Launch on TestCompany"
- main:
  - heading "Built for TestCompany" [level=2]
  - paragraph: Exclusive preview of your white-label solar intelligence tool
  - text: ⚡ Ready to launch on TestCompany.com T ✓
  - heading "TestCompany Solar Intelligence — Live in 10 Minutes" [level=1]
  - paragraph: Embed a branded quote tool on TestCompany.com. Turn visitors into booked calls. 14-day refund if it doesn't lift conversions.
  - heading "Enter Your Property Address" [level=2]
  - paragraph: Get a comprehensive solar analysis tailored to your specific location
  - textbox "Start typing your property address..."
  - paragraph: Enter your property address to get started
  - button "Launch on TestCompany":
    - text: Launch on TestCompany
    - img
  - paragraph: "Preview: 2 runs left."
  - paragraph: Expires in 6d 23h 59m 53s
  - text: 50K+ Properties Analyzed — Total Savings Generated 98% Accuracy Rate 24/7 AI Support
  - img
  - text: Advanced Analytics AI-powered insights with 25-year projections and ROI analysis
  - img
  - text: Premium Network Connect with verified, top-rated solar installers in your area
  - img
  - text: Enterprise Security Bank-level encryption and SOC 2 compliance for your data Used by 50+ installers
  - heading "How It Works" [level=2]
  - text: "1"
  - heading "Add the widget" [level=3]
  - paragraph: One line of code to embed on your website
  - text: "2"
  - heading "Visitors get instant quotes" [level=3]
  - paragraph: AI-powered analysis in seconds
  - text: "3"
  - heading "Your team gets booked calls" [level=3]
  - paragraph: Qualified leads ready to convert
  - heading "Frequently Asked Questions" [level=2]
  - heading "CMS? — Yes, 1-line <script>. Hosted option too." [level=3]
  - paragraph: Works with any website platform. Just add one line of code.
  - heading "Accuracy? — NREL PVWatts v8 • EIA rates • local irradiance" [level=3]
  - paragraph:
    - text: Industry-standard data sources.
    - link "View methodology":
      - /url: /methodology
    - text: .
  - heading "Security? — Encrypted in transit & at rest" [level=3]
  - paragraph: Bank-level security for all customer data.
  - heading "Cancel? — Yes, 14-day refund if it doesn't lift booked calls" [level=3]
  - paragraph: No long-term contracts. Cancel anytime.
  - heading "Support? — Email support 24/7" [level=3]
  - paragraph: Get help whenever you need it.
- contentinfo:
  - text: Estimates generated using NREL PVWatts® v8.•Mapping & location data © Google
  - button "Terms"
  - button "Privacy"
  - link "Methodology":
    - /url: /methodology
  - text: •Powered by Sunspire
- button "Add to TestCompany"
- button "Copy link"
- text: Trusted by 50+ Solar Brands Bank-Level Security • SOC 2 Compliant • 98% Accuracy
- alert
```