...
function getBase(): Airtable.Base {
  if (!base) {
    ...
    base = new Airtable({ apiKey: ENV.AIRTABLE_API_KEY }).base(
...
export async function upsertLeadByEmailAndTenant(
  ...
  if (found[0]) {
    const updated = await base(TABLES.LEADS).update([
      { id: found[0].id!, fields: merged },
    ]);
    return {
      id: updated[0].id,
      ...updated[0].fields,
...
export async function upsertLeadSuppressionByEmail(
  email: string,
  source: string,
): Promise<void> {
  try {
...
export async function setTenantDomainStatus(
  handle: string,
  status: string,
): Promise<void> {
  try {
...
export async function setRequestedDomain(
  handle: string,
  domain: string,
): Promise<void> {
  try {
