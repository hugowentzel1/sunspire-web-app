...
export async function getProduction(request: PVWattsRequest): Promise<PVWattsResponse & { degraded?: boolean }> {
  const {
    lat,
    lng,
    dc_kw,
...
async function retryWithBackoff<T>(
  ...
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (error instanceof Error && error.message.includes('PVWatts API error:')) {
...
function calculateFallbackProduction(request: PVWattsRequest): PVWattsResponse {
  const { lat, lng, dc_kw, tilt = 20, azimuth = 180, losses = ENV.DEFAULT_LOSSES_PCT } = request;
  ...
  const dailyProduction = dc_kw * baseIrradiance * tiltFactor * azimuthFactor * systemEfficiency;
  const annualProduction = dailyProduction * 365;
  
  // Monthly distribution (rough approximation)
  const monthlyFactors = [0.6, 0.7, 0.9, 1.1, 1.2, 1.3, 1.3, 1.2, 1.0, 0.8, 0.6, 0.5];
  const ac_monthly = monthlyFactors.map(factor => annualProduction * factor / 12);
