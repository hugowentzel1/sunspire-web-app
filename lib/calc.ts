...
export async function calculateSolarEstimate(
  ...
  const degradationPercentage = parseFloat(
    process.env.DEFAULT_DEGRADATION_PCT || "0.5",
  );
  ...
  const systemSizeKW =
    Math.round(
      (targetProduction / (solarIrradiance * 365 * efficiencyFactor)) * 10,
    ) / 10;
...
export const methodology = {
  ...
  description:
    "Our estimates are based on industry-standard data and real-world performance metrics.",
  sources: [
