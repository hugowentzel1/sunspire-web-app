...
test("Paid: no demo CTAs or CRM chips, footer present", async ({ page }) => {
  ...
  // no demo-only text
  await expect(
    page.getByText(/Activate on Your Domain|Unlock Full Report|CRM-ready/i),
  ).toHaveCount(0);
...
test("Paid report: banner removed", async ({ page }) => {
  const url = LIVE_PAID.replace(
    "/?",
    "/report?address=123+Main+St&lat=40.7128&lng=-74.0060&placeId=test",
  ); // same params, report page, still paid
  await page.goto(url, { waitUntil: "networkidle" });
  await expect(
    page.getByText(
      /Ready to Launch|Unlock Full Report|Activate on Your Domain/i,
    ),
  ).toHaveCount(0);
  await expect(page.locator("[data-paid-footer]")).toBeVisible();
});
...
test("Paid footer: 3-column layout with company branding", async ({ page }) => {
  ...
  await expect(
    footer.locator('img[alt*="SolarPro Energy logo"]'),
  ).toBeVisible();
  ...
  await expect(
    footer.getByText("Estimates are informational only"),
  ).toBeVisible();
