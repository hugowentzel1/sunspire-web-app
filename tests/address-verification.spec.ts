...
test.describe('Address Verification', () => {
  test('footer shows new address', async ({ page }) => {
    await page.goto(`${LIVE_BASE}/?company=qa-acme`, { waitUntil: 'networkidle' });
    
    // Check that the new address appears in the footer
    await expect(page.getByText('1700 Northside Drive Suite A7 #5164 Atlanta, GA 30318')).toBeVisible();
    
    // Verify old address is not present
    await expect(page.getByText('3133 Maple Dr Ne Ste 240 #1156 Atlanta, GA 30305')).toHaveCount(0);
  });

  test('privacy page shows new address', async ({ page }) => {
    await page.goto(`${LIVE_BASE}/privacy`, { waitUntil: 'networkidle' });
    
    // Check that the new address appears in the privacy page
    await expect(page.getByText('1700 Northside Drive Suite A7 #5164 Atlanta, GA 30318')).toBeVisible();
  ...
  test('terms page shows new address', async ({ page }) => {
    ...
    // Check that the new address appears in the terms page
    await expect(page.getByText('1700 Northside Drive Suite A7 #5164 Atlanta, GA 30318')).toBeVisible();
