...
test.describe("Final Live Site Verification", () => {
  test("Homepage with Apple branding loads correctly", async ({ page }) => {
    ...
    const ctaButtons = page
      .locator("button, a")
      .filter({ hasText: /Get Quote|Start|Try|Demo/i });
  ...
  test("Terms page has refunds section but no last updated text", async ({
    page,
  }) => {
    ...
    await expect(page.getByText("14-day pilot guarantee")).toBeVisible();
    await expect(
      page.getByText("support@getsunspire.com").first(),
    ).toBeVisible();
  ...
  test("DPA page has no last updated text", async ({ page }) => {
    ...
    await expect(page.locator("h1").first()).toBeVisible();
    await expect(
      page.getByText("Data Processing Agreement").first(),
    ).toBeVisible();
  });
  ...
  test("Documentation pages have centered bullet points", async ({ page }) => {
    ...
    await expect(page.locator("h1").first()).toBeVisible();
    await expect(
      page.getByText("HubSpot CRM Integration").first(),
    ).toBeVisible();
  });
  ...
  test("Domain onboarding still works", async ({ page }) => {
    await page.goto(
      `${LIVE_URL}/onboard/domain?tenant=Apple&companyWebsite=apple.com`,
    );
  ...
  test("API endpoints respond correctly", async ({ page }) => {
    ...
    const autocompleteResponse = await page.request.get(
      `${LIVE_URL}/api/autocomplete?query=123`,
    );
  ...
  test("Take final screenshots for verification", async ({ page }) => {
    ...
    await page.screenshot({
      path: "final-refund-redirect.png",
      fullPage: true,
    });

    console.log(
      "Screenshots saved: final-homepage.png, final-terms.png, final-refund-redirect.png",
    );
  });
});
