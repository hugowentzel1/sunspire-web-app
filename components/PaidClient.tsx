...
function PaidClient({ buildSha }: PaidClientProps) {
  console.log('[paid] render start');
  ...
  useEffect(() => { 
    console.log('[paid] hydrated');
    (window as any).__CONTENT_SHOWN__ = true; 
  }, []);

  // Check if we should redirect to demo version if no company
  useEffect(() => {
    const timer = setTimeout(() => {
      if (isDemo && typeof window !== 'undefined') {
        // Redirect to demo version with all URL parameters
  ...
  // Early return for demo versions to prevent paid content from rendering
  if (isDemo) {
    return <div>Redirecting to demo version...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-100 font-inter">
      ...
      <div className="bg-gray-900 text-white py-12">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <p className="text-gray-400">&copy; 2024 Sunspire. All rights reserved.</p>
          </div>
        </div>
      </div>
    </div>
