...
async function handleViewTracking(req: AuthenticatedRequest): Promise<NextResponse> {
  try {
    const body = await req.json();
    const validatedData = viewTrackingSchema.parse(body);
    
    const { email, campaignId, address } = validatedData;
    const { tenant } = req;
    
    logger.info('View tracking request:', {
    ...
    if (email) {
      ...
      logger.info('Lead upserted for view tracking:', { email, tenant: tenant.handle });
    }
...
export const POST = withTenantScope(handleViewTracking);
