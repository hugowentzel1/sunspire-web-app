      ...
      <main data-testid="report-page" className="py-12">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="space-y-10">
          <div className="flex flex-col items-center gap-6">
            ...
            {demoMode ? (
              <div className="mx-auto mt-2 grid grid-cols-[1fr,auto,1fr] items-center gap-8 text-sm text-gray-500" style={{ maxWidth: '64ch', fontFeatureSettings: '"tnum"' }}>
                <span className="justify-self-end text-right inline-block min-w-[14ch] opacity-80">Generated {formatDateSafe(estimate.date)}</span>
                <span className="justify-self-center text-center inline-block font-medium text-gray-900">Runs left: {Math.abs(remaining)}</span>
                <span className="justify-self-start text-left inline-block min-w-[14ch] opacity-80">Expires in {countdown.days}d {countdown.hours}h {countdown.minutes}m {countdown.seconds}s</span>
              </div>
            ) : (
