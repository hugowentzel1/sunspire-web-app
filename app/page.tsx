...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (!isDemo) {
      ...
      if (company) {
        // Build paid URL with all parameters
        const paidParams = new URLSearchParams();
        paidParams.set('company', company);
        if (brandColor) paidParams.set('brandColor', brandColor);
        if (logo) paidParams.set('logo', logo);
        
        // Add any other parameters
        searchParams.forEach((value, key) => {
          if (key !== 'demo' && !paidParams.has(key)) {
            paidParams.set(key, value);
          }
        });
        
        router.replace(`/paid?${paidParams.toString()}`);
      }
    }
  }, [router, searchParams]);

  // Demo version content (from commit 19610abb0fc9042eb7ff822f21586178043fcd53)
  return <DemoVersion />;
}

// Demo version component (exact copy from commit 19610abb0fc9042eb7ff822f21586178043fcd53)
function DemoVersion() {
  const { useState, useEffect } = require('react');
  const { useRouter } = require('next/navigation');
  const dynamic = require('next/dynamic').default;
  const { PlaceResult } = require('@/lib/calc');
  const LegalFooter = require('@/components/legal/LegalFooter').default;
  const { useBrandTakeover } = require('@/src/brand/useBrandTakeover');
  const HeroBrand = require('@/src/brand/HeroBrand').default;
  const { useBrandColors } = require('@/hooks/useBrandColors');
  const { usePreviewQuota } = require('@/src/demo/usePreviewQuota');
  const { useCountdown } = require('@/src/demo/useCountdown');
  const { useIsDemo } = require('@/src/lib/isDemo');
  const React = require('react');
  const { attachCheckoutHandlers } = require('@/src/lib/checkout');
  const { tid } = require('@/src/lib/testids');
          ...
          <div className="text-center space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <button 
                    ...
                    data-cta-button
                    {...tid('cta-primary')}
                    className={`w-full ${!address.trim() || isLoading ? 'btn-disabled' : 'btn-cta'}`}
                  >
                    {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-w-5xl mx-auto section-spacing">
              ...
              <div className="feature-card p-5 text-center">
                ...
                <div className="desc">HubSpot, Salesforce, Pipedrive, and more</div>
              </div>
              <div className="feature-card p-5 text-center">
                <div className="w-12 h-12 mx-auto bg-gradient-to-br from-[var(--brand-primary)] to-white rounded-2xl flex items-center justify-center shadow-lg">                                                         
            ...
            <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-8 text-center border border-gray-200/50 max-w-3xl mx-auto section-spacing">
              <div className="space-y-4">
                <h3 className="text-2xl font-bold text-gray-900">Ready to get started?</h3>
                <p className="text-gray-600">Join dozens of solar companies already using our platform</p>
