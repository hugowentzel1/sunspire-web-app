...
if (!process.env.VERCEL_TOKEN || !process.env.VERCEL_PROJECT_ID) {
  console.error('❌ VERCEL_TOKEN and VERCEL_PROJECT_ID environment variables required');
  process.exit(1);
}
...
async function testDomainFlow() {
  ...
  try {
    const statusResponse = await makeRequest(`${BASE_URL}/api/domains/status?domain=${TEST_DOMAIN}`);
  ...
  console.log('   1. Add CNAME record: test.example.com CNAME sunspire-web-app.vercel.app');
