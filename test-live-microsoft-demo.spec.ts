...
test.describe('Live Microsoft Demo - Complete Verification', () => {
  ...
  test('Verify Microsoft branding and color consistency', async ({ page }) => {
    ...
    const microsoftLogo = page.locator('img[alt*="Microsoft"], img[src*="microsoft"]');
    ...
    // Check for consistent Microsoft blue theme
    expect(buttonColor.backgroundColor).toContain('rgb(0, 120, 212)'); // Microsoft blue
  });

  test('Test all CTA buttons redirect to Stripe', async ({ page }) => {
    // Find all CTA buttons that should redirect to Stripe
    const ctaButtons = page.locator('button:has-text("Get Started"), button:has-text("Unlock"), button:has-text("Upgrade"), button:has-text("Subscribe")');
  ...
  test('Verify complete color consistency', async ({ page }) => {
    ...
    // Check for Microsoft brand consistency
    const microsoftBlue = 'rgb(0, 120, 212)';
  ...
  test('Test complete user journey', async ({ page }) => {
    ...
    await expect(page.locator('img[alt*="Microsoft"], img[src*="microsoft"]')).toBeVisible();
